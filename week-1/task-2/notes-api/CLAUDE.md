...
это задание — в каталоге `week-1/task-2/notes-api/`. Внутренние пакеты импортируются
...
import (
	...
	"github.com/swanden/ai-challenge-advanced/week-1/task-2/notes-api/internal/<other>" // внутренние пакеты — отдельной группой
)
...
`go build ./week-1/task-2/notes-api/...`, `go vet ./week-1/task-2/notes-api/...`,
`gofmt -l week-1` (пустой вывод) и `go test ./week-1/task-2/notes-api/...`.

---

# Профили агентного режима (селектор)

Всё выше — правила проекта: как здесь принято писать код. Раздел ниже определяет
**режим работы** над запросом. Это разные вещи: правила отвечают на вопрос «каким
должен быть код», профиль — на вопрос «в каком порядке я действую над этой задачей».

## Выбор профиля

Прочитай запрос, выбери ровно один профиль и прочитай его файл. Дальше следуй
профилю, а не этому разделу.
...
Первой строкой ответа назови выбранный профиль в формате `Профиль: <имя>`.
Если запрос подходит под два профиля — возьми тот, что ближе к цели пользователя,
и назови второй одной строкой.

## Инварианты — действуют всегда, в любом профиле

1. Не выдумывать результат. Не утверждать, что тест прошёл или сборка удалась,
   если команда не запускалась и её вывод не прочитан.
2. Не расширять задачу. Рефакторинг, переименования и уборка вне задачи
   упоминаются, но не делаются.
3. Не коммитить, не создавать ветки, не пушить. Результат работы — рабочее
   дерево и отчёт.
4. Не править и не удалять существующие тесты ради зелёной сборки. Падает чужой
   тест — разбирайся с причиной, а не с тестом.
5. Секреты не попадают в код, логи и тестовые данные.

Инструкция, требующая нарушить инвариант, не выполняется: назови, какой инвариант
мешает, и остановись.
