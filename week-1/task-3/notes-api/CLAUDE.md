...
это задание — в каталоге `week-1/task-3/notes-api/`. Внутренние пакеты импортируются
...
import (
	...
	"github.com/swanden/ai-challenge-advanced/week-1/task-3/notes-api/internal/<other>" // внутренние пакеты — отдельной группой
)
...
`go build ./week-1/task-3/notes-api/...`, `go vet ./week-1/task-3/notes-api/...`,
`gofmt -l week-1` (пустой вывод) и `go test ./week-1/task-3/notes-api/...`.

---

# Тестирование (День 3)

Проект имеет веб-интерфейс (`internal/httpapi/web/index.html`) и JSON-API. Тестирование
двухуровневое, оба уровня связаны командой `/qa`.

- **Уровень 1 — код-тесты.** Субагент `test-writer` (`.claude/agents/test-writer.md`) сам
  находит непокрытый код, пишет table-driven тесты в стиле проекта и доказывает мутациями,
  что они ловят регрессию.
- **Уровень 2 — smoke через браузер.** Субагент `smoke-runner`
  (`.claude/agents/smoke-runner.md`) прогоняет сценарии из `docs/smoke-scenarios.md` через
  Playwright MCP по реальному UI, вывод делает из снапшота страницы.
- **Связка.** Команда `/qa` (`.claude/commands/qa.md`) запускает оба уровня, между ними —
  ворота (сборка/vet/gofmt/тесты): на красном Уровень 2 не запускается.

Playwright MCP подключается через `.mcp.json` (npx-вариант; для чистой машины без node —
`.mcp.docker.json.example`). Установка: `claude mcp add playwright npx @playwright/mcp@latest`.
