...
//
// Отдельный эндпоинт, не chat/completions: модели эмбеддингов не
// генерируют текст, а отдают вектор фиксированной длины, и стоит это
// на порядок дешевле генерации.
func (c *Client) Embed(text string) ([]float64, error) {
	...
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("HTTP %d: %s", resp.StatusCode, cut(string(raw), 300))
	}
	var parsed struct {
		Data []struct {
	...
	if err := json.Unmarshal(raw, &parsed); err != nil {
		return nil, fmt.Errorf("не разобрал ответ: %w", err)
	}
